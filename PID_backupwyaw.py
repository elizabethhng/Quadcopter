...
class FrontEnd(object):
	...
	def run(self):
		...
		#aruco dictionary
		aruco_dict = cv2.aruco.Dictionary_get(cv2.aruco.DICT_6X6_250) #markers w/ id 1-1000 may be used
		...
		while not should_stop:
			...
			if res[1] != None: # if aruco marker detected
				...
				if xyz[0] < desired_xyz[0] + land_xyz[0] and xyz[0] > desired_xyz[0] - land_xyz[0] and xyz[1] < desired_xyz[1] + land_xyz[1] and xyz[1] >desired_xyz[1] - land_xyz[1] and roff>-5 and roff<5:
					...
					# self.update()
					self.left_right_velocity, self.for_back_velocity, self.yaw_velocity = 0,0,0
				...
				print("off_x: -", int(xyz[0]), self.left_right_velocity,"off_y:", int(xyz[1]), self.for_back_velocity, "off_z:", int(xyz[2]), self.for_back_velocity, "off_r:",  self.yaw_velocity, sep="\t")
